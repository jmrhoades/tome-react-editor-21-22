...
export const MetricsProvider = ({ children }) => {
	...
	*/
	...
	const getPageTop = page => {
		...
		// 	const cH = getContentHeight(page);
		// 	console.log(dPageTop, cH, window.innerHeight, window.pageYOffset)
		// } else {
		// 	tempPageTop.set(dPageTop);
		// }
		// dPageTop = -1700;

		//if (pageTop < metricConstants.cPageMinY) pageTop = metricConstants.cPageMinY;

		if (rowResizing && rowResizing.isResizingWidth) {
			const heightDifference = pageHeight - rowResizing.tempPageHeight;
			//pageTop = scrollY - heightDifference / 2;
			pageTop = scrollY - heightDifference * rowResizing.handleYPercent;
			//console.log(rowResizing.height, pageHeight, rowResizing.tempPageHeight, heightDifference);
	...
	const setScrollY = (y) => {
		console.log("setScrollY", y, metricConstants.cPageMinY);
		const min = metricConstants.cPageMinY;
		if (y > min) y = min;
		//pageScrollY.set(y);
	}

	const animatePageYForWidthChange = (page) => {
		const pageHeight = getPageHeight(page);
		const heightDifference = pageHeight - rowResizing.tempPageHeight;

		 let newScrollY = rowResizing.cachedScrollY - heightDifference * rowResizing.handleYPercent;
		 
		console.log("animatePageYForWidthChange", pageScrollY.get(), rowResizing.cachedScrollY , heightDifference, rowResizing.handleYPercent, newScrollY)
...
/*
	// Mouse wheel attempt 3
	useEffect(() => {
		const handleWheel = e => {
			//const direction = e.deltaY > 0 ? -1 : 1;
			// pageScrollY.set(pageScrollY.get() - e.deltaY);
			setScrollY(pageScrollY.get() - e.deltaY)
		};
